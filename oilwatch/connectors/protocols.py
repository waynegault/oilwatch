...
class PageLike(Protocol):
    ...
    #: Playwright's own ``Page.evaluate(expression, arg=None)``. Declared with the
    #: single argument the connectors pass, which the real method and the fake both
    #: accept. A script is the only way to read a value a page set as a *property*:
    #: serialised HTML carries the markup, not the current value of an input.
    async def evaluate(self, expression: str) -> Any: ...
