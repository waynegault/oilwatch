...
| 8.8 | 🔧 Pyright is clean on the files you touched | `.venv\Scripts\python.exe -m pyright <changed files>` | Zero errors on those files. Pylance *is* Pyright, so this is the engine the editor runs. `pyrightconfig.json` supplies the venv and includes `oilwatch`, so bare `pyright` reports the package while `pyright tests` reports the tree. Chase the files you touched, not the tree's backlog |
| 8.9 | 🔍 The nine package errors that remain are accepted artifacts | `.venv\Scripts\python.exe -m pyright` | Bare `pyright` reports exactly nine errors and one warning, none fixable without casting working code, so they are recorded rather than chased: `analytics.py` ×4 (matplotlib's `plot`/`legend` do not accept `list[datetime]`/`list[object]`); `geo.py` ×4 (geopy *generates* `geocode` with a decorator that switches on the adapter — `geopy/geocoders/base.py` has `def geocode` commented out and a decorator testing `BaseAsyncAdapter` — so a synchronous call reads as a coroutine); `import_xls.py` ×1 (xlrd wants `Literal[0, 1]` for `datemode`). The warning is pyright's `reportUnsupportedDunderAll` on the `*_CONNECTORS` unpacking in §8.7 |
...
| 10.2 | 🔍 Coverage at or above the floor | `.venv\Scripts\python.exe -m coverage report -m` | Total ≥ 94 (measured 95) |
...
| 10.4 | 🔧 A fake matches the real library | Read the fake for a changed connector | It accepts the real signature and models the real value (a control's pre-filled value, the DOM set actually used) — a fake that returns `None` for every lookup makes a path silently untested |
