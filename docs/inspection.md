...
| 2.11 | 🔍 No accidental key-shaped literals | `grep -rnIE "[A-Za-z0-9_-]{32,}" oilwatch/` | Every hit is a literal, not a credential. The pattern matches any long token, so on 2026-09-18 the hits were Chrome flags and CSS selectors (`--disable-features=AutofillServerCommunication,AutofillEnableAccountWalletStorage`, `#sgcPriceChecker_txtQuotePostcode`) plus the public Microsoft OAuth client id, which is already published in `monitor_email.bat`. **`-I` matters**: without it the compiled `oilwatch/**/__pycache__/*.pyc` files match and each reports "binary file matches", so the pass starts with nine lines of noise about files that are not source. Re-run 2026-09-24: six source lines, the same five distinct tokens. The check is that none is a key — not that there are none — so read a hit before calling it benign |
...
| 3.10 | 🔧 A supplier's phone is data, never a route | `grep -rnE "order_channel\|phone_email\|by phone" oilwatch/`, then read the manual notes and `oilwatch status` | Nothing **offers** phoning a supplier to get a quote: no `phone`/`phone_email` `order_channel` (a number alone reports `none`) and no connector note or live `status` `note` carrying a supplier's number. (The `order_notes` attributes this row used to police are gone: nothing read them, so they were three strings waiting for a reader that never came.) The number itself **stays** in `config/suppliers.json`, the DB `phone` column, `contact.phone` and `raw_payload` — deliberately, because the owner wants to know a supplier's number and the app must never act on it. A literal "no digits anywhere" bar is the wrong one and contradicts AGENTS.md's "the app never rings anyone" |
...
| 12.6 | 🔧 Every generated artifact is rebuilt in the same commit as the change that moved it | `.venv\Scripts\python.exe tools\build_snapshot.py --check` **and** `.venv\Scripts\python.exe tools\dump_schema.py --check` **and** `.venv\Scripts\python.exe tools\make_icon.py --check` | All three answer "in step". There are three such artifacts, and they answer different questions: `data/oilwatch-history.sqlite` (what the prices were, built by `tools/build_snapshot.py`), `docs/schema.sql` (what shape the database is, built by `tools/dump_schema.py`) and `assets/oilwatch.ico` (the shortcut's artwork, drawn by `tools/make_icon.py`; its `--check` compares the frames it carries, not its bytes, so a Pillow upgrade cannot fail it). Nothing writes any of them as a side effect of anything else, so each lags its source silently — and for the *export* no test can catch that, because a checkout has the export and never the live database, which is the one file deliberately not committed. That is also why only the schema file has a guard test (`tests/test_schema_dump.py`): where a test is possible it exists, and where it is not, this row and `--check` are the guard. On 2026-09-24 the export went stale three times in one session — a deduplication, ValueOils' phone correction, and the note describing that correction — and the phone corrected a *value* while changing no count, which is why its `--check` compares every table by row count **and** by a digest of its kept columns, skipping only the clock readings named in `_VOLATILE_COLUMNS` |
